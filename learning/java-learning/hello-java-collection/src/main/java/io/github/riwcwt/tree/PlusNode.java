...
public class PlusNode {
     ...
     * 父节点
     */
    protected PlusNode parent;

    /**
     * 叶节点的前节点
     */
    protected PlusNode previous;

    /**
     * 叶节点的后节点
     */
    protected PlusNode next;
     ...
     * 子节点
     */
    protected List<PlusNode> children;

    public PlusNode(boolean isLeaf) {
        this.isLeaf = isLeaf;
        entries = new ArrayList<Entry<Comparable, Object>>();

        if (!isLeaf) {
            children = new ArrayList<PlusNode>();
        }
    }

    public PlusNode(boolean isLeaf, boolean isRoot) {
        this(isLeaf);
        this.isRoot = isRoot;
    }
    ...
    public void insertOrUpdate(Comparable key, Object obj, BplusTree tree) {
        ...
        if (isLeaf) {
            ...
            if (contains(key) || entries.size() < tree.getOrder()) {
                ...
                //分裂成左右两个节点
                PlusNode left = new PlusNode(true);
                PlusNode right = new PlusNode(true);
                //设置链接
                if (previous != null) {
                ...
                if (parent != null) {
                    ...
                    isRoot = false;
                    PlusNode parent = new PlusNode(false, true);
                    tree.setRoot(parent);
                    left.setParent(parent);
                    right.setParent(parent);
    ...
    protected void updateInsert(BplusTree tree) {
        ...
        if (children.size() > tree.getOrder()) {
            //分裂成左右两个节点
            PlusNode left = new PlusNode(false);
            PlusNode right = new PlusNode(false);
            //左右两个节点关键字长度
            int leftSize = (tree.getOrder() + 1) / 2 + (tree.getOrder() + 1) % 2;
            int rightSize = (tree.getOrder() + 1) / 2;
     ...
     * 调整节点关键字
     */
    protected static void validate(PlusNode node, BplusTree tree) {
    ...
    protected void updateRemove(BplusTree tree) {
        ...
        if (children.size() < tree.getOrder() / 2 || children.size() < 2) {
            if (isRoot) {
                ...
                PlusNode previous = null, next = null;
                if (prevIdx >= 0) {
                ...
                if (previous != null
                    ...
                    PlusNode borrow = previous.getChildren().get(idx);
                    ...
                    //后叶子节点首位添加到末尾
                    PlusNode borrow = next.getChildren().get(0);
                    ...
                    if (previous != null
                        ...
                        for (int i = previous.getChildren().size() - 1; i >= 0; i--) {
                            PlusNode child = previous.getChildren().get(i);
                        ...
                        for (int i = 0; i < next.getChildren().size(); i++) {
                            PlusNode child = next.getChildren().get(i);
                            children.add(child);
                            child.setParent(this);
                        }
    ...
    public PlusNode getPrevious() {
        return previous;
    }

    public void setPrevious(PlusNode previous) {
        this.previous = previous;
    }

    public PlusNode getNext() {
        return next;
    }

    public void setNext(PlusNode next) {
        this.next = next;
    }
    ...
    public PlusNode getParent() {
        return parent;
    }

    public void setParent(PlusNode parent) {
        this.parent = parent;
    }
    ...
    public List<PlusNode> getChildren() {
        return children;
    }

    public void setChildren(List<PlusNode> children) {
