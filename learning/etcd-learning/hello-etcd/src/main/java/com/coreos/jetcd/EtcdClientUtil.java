...
public class EtcdClientUtil {
	...
	static final <T extends AbstractStub<T>> T configureStub(T stub, Optional<String> token) {
		return token.map(t -> {
			...
			Metadata.Key<String> TOKEN_KEY = Metadata.Key.of(EtcdConstants.TOKEN, Metadata.ASCII_STRING_MARSHALLER);
	...
	static final NameResolver.Factory simpleNameResolveFactory(List<String> endpoints) {
		return new SimpleEtcdNameResolverFactory(endpoints.stream().map(EtcdClientUtil::endpointToUri).collect(Collectors.toList()));
	}
	...
	static ManagedChannelBuilder<?> defaultChannelBuilder(NameResolver.Factory factory) {
		return ManagedChannelBuilder.forTarget("etcd").nameResolverFactory(factory).usePlaintext(true);
	}
}
